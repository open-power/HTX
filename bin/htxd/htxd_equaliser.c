...
int num_tests = 0, total_tests=0, num_cpus, smt_val;
int pattern_length = UTILIZATION_QUEUE_LENGTH;
int length_flag = 0;

unsigned long long sys_cpu_util_pattern[MAX_THREADS] = {0};
unsigned long long offline_cpu_pattern[MAX_THREADS] = {0};
...
void print_util_seq()
{
    ...
    #ifdef __HTX_LINUX__
        fprintf(util_fp, "\toffline_cpu:\t");
        bin(offline_cpu_pattern[cpu_num]);
    #endif
    }
    fflush(util_fp);
}
...
{
    int num_inactive_cpus, core_num, cur_cpu_util_index = 0;
    int i, len, cpu, inactive_cpus_num[MAX_CPUS_PER_CORE];
    int num_of_cpus, cpu_num;
#ifdef __HTX_LINUX__
    int num_offline_cpus = 0, offline_cpu_num = 0;
#endif
    unsigned long long bit_value;
    time_t cur_time;

    for (i = 0; i < num_cpus; i++) {
        sys_cpu_util_pattern[i] = (1ull << pattern_length) - 1; /* initializing all bits to 1 for pattern length */
    #ifdef __HTX_LINUX__
        offline_cpu_pattern[i] = 0;
    #endif
    }
    /* printf("inside generate_sys_util_pattern - cur_sys_cpu_util_prcnt: %d, pattern_length: %d\n", cur_sys_cpu_util_prcnt, pattern_length); */
    num_inactive_cpus = num_cpus - ((cur_sys_cpu_util_prcnt * num_cpus) / 100);
    if (num_inactive_cpus == num_cpus) {
		num_inactive_cpus--;
	}
#ifdef __HTX_LINUX__
    num_offline_cpus = (test_config.offline_cpu_prcnt * num_inactive_cpus) / 100;
#endif
    if (test_config.sys_util_flag == 1) {
		num_inactive_cpus = (num_inactive_cpus / smt_val) * smt_val;
    #ifdef __HTX_LINUX__
        num_offline_cpus = (num_offline_cpus / smt_val) * smt_val;
    #endif
    }

    for (len = 0; len < pattern_length; len++, cur_cpu_util_index++) {
        for (cpu = 0; cpu < num_inactive_cpus; ) {
            do {
                bit_value = 1;
                if (test_config.sys_util_flag == 1) {
                    core_num = rand() % sys_stat.cores;
                    num_of_cpus = get_cpus_in_core(core_num, inactive_cpus_num);
                } else {
                    inactive_cpus_num[0] = rand() % num_cpus;
                    num_of_cpus = 1;
                }
                bit_value = (sys_cpu_util_pattern[inactive_cpus_num[0]] >> cur_cpu_util_index) & 0x1;
            } while (bit_value != 1);  /* bit value is used to make sure we don't pick the same cpu while randomly picking */
            for (cpu_num = 0; cpu_num < num_of_cpus; cpu_num++, cpu++) {
                sys_cpu_util_pattern[inactive_cpus_num[cpu_num]] &= (~(1ull << cur_cpu_util_index));
            #ifdef __HTX_LINUX__
                if (offline_cpu_num < num_offline_cpus) {
                    offline_cpu_pattern[inactive_cpus_num[cpu_num]] |= (1ull << cur_cpu_util_index);
                    offline_cpu_num++;
				}
            #endif
            }
             ...
             for (core_num = 0; core_num < sys_detail.nodes[node_num].chips[chip_num].num_cores; core_num++) {
                 for (cpu_num= 0; cpu_num < sys_detail.nodes[node_num].chips[chip_num].cores[core_num].num_cpus; cpu_num++) {
                     ...
                     if (num_tests_configured != 0) {
                         ...
                         #ifdef __HTX_LINUX__
                             test_config.exer_config[index].config.offline_cpu = offline_cpu_pattern[lcpu];
                         #endif
                             test_config.exer_config[index].config.util_pattern = UTIL_SYS;
...
void update_exer_info()
{
    ...
    if (test_config.sys_util_flag || test_config.offline_cpu_prcnt) {
                    ...
                    for (core_num = 0; core_num < sys_detail.nodes[node_num].chips[chip_num].num_cores; core_num++) {
                        for (cpu_num= 0; cpu_num < sys_detail.nodes[node_num].chips[chip_num].cores[core_num].num_cpus; cpu_num++) {
                            ...
                            if (lcpu == bind_cpu_num) {
                                ...
                                #ifdef __HTX_LINUX__
                                    test_config.exer_config[k].config.offline_cpu = offline_cpu_pattern[lcpu];
                                #endif
                                    test_config.exer_config[k].config.util_pattern = UTIL_SYS;
                                    test_config.exer_config[k].config.pattern_length = pattern_length;
                                }
...
void populate_run_time_data(int index)
{
    run_time_config *exer_conf;
    ...
    shm_addr = htxd_get_equaliser_shm_addr();
    (shm_addr.hdr_addr)++;         /* skip over header                  */
    p_htxshm_HE = shm_addr.HE_addr;

    exer_conf = test_config.exer_config;
    exer_conf[index].data.target_utilization = exer_conf[index].config.utilization_sequence[0];
    if (exer_conf[index].config.util_pattern != UTIL_PATTERN && exer_conf[index].config.util_pattern != UTIL_SYS) {
...
void initialize_run_time_data(void)
{
    ...
    if (test_config.sys_util_flag || test_config.offline_cpu_prcnt) {
        ...
        for (index = 0; index < shm_addr.hdr_addr->max_entries; index++) {
            populate_run_time_data(index);
        }
    }
}

void equaliser_action (int index, int lcpu, int pcpu)
{
    char str1[128], msg[256];
    int rc, kill_return_code = 0, count; /* loop counter     */
    int action, semhe_id;
    struct htxshm_HE *p_htxshm_HE;  /* pointer to a htxshm_HE struct     */
    struct semid_ds sembuffer;      /* semaphore buffer                  */
    union shm_pointers shm_addr, shm_addr_wk; /* work ptr into shm                 */
    long clock;
    struct tm new_time;
    char str_time[50];
    uint64 tick1, tick2;
    ...
    if (((p_htxshm_HE + index)->PID != 0 ) &&     /* Not Dead?  */
        ...
        if ((action == 0) && ((p_htxshm_HE + index)->equaliser_halt == 0)) {
            ...
            #ifdef __HTX_LINUX__
                if (test_config.exer_config[index].config.offline_cpu) {
                    ...
                    rc = system(str1);
                    if (!WIFEXITED(rc)) {
                        sprintf(msg, "cmd to offline the cpu %d could not be executed.\n", pcpu);
            ...
            if (test_config.exer_config[index].config.offline_cpu) {
                ...
                rc = system(str1);
                if (!WIFEXITED(rc)) {
                    sprintf(msg, "cmd to online the cpu %d could not be executed.\n", pcpu);
                    htxd_send_message(msg, 0, HTX_SYS_SOFT_ERROR, HTX_SYS_MSG);
                } else {
                    if (WEXITSTATUS(rc)) {
						read_tb(tick1);
                    	...
                    	if (rc < 0) {
                        	...
                        	if (rc == -2) {
                            	...
                            	if (rc < 0) {
                                	sprintf(msg, "After retry also (tick diff: %lld), could not bind process %d to cpu %d. rc= %d.\n",
                                    	         (tick2 - tick1), (int)(p_htxshm_HE + index)->PID, pcpu, rc);
                                	htxd_send_message(msg, 0, HTX_SYS_SOFT_ERROR, HTX_SYS_MSG);
                            	} else {
                                	sprintf (msg, "Took %lld ticks to bind the cpu after bringing it online.\n", (tick2 - tick1));
                                	htxd_send_message(msg, 0, HTX_SYS_SOFT_ERROR, HTX_SYS_MSG);
								}
					 		}
                        }
...
void htxd_equaliser(void)
{
   ...
   */
    ...
    char workstr[512], msg[256];
    int i, rc, lcpu, pcpu; /* loop counter     */
    int node_num, chip_num, core_num, cpu_num;
    int num_tests_configured, index;
    useconds_t micro_seconds;
    char global_htxd_log_dir[256] = "/tmp/";
    long clock, tm_log_save= 0;
    struct stat buf1;
   ...
   */
    ...
    smt_val = sys_stat.cpus / sys_stat.cores;
   ...
   */
    ...
    bzero (test_config.exer_config, (sizeof(run_time_config) * (shm_addr.hdr_addr)->max_entries));
    ...
    test_config.offline_cpu_prcnt = htxd_get_equaliser_offline_cpu_flag();
  ...
  /*********************************************************************************/
	...
	while(1) {
		if (test_config.sys_util_flag || test_config.offline_cpu_prcnt) {
                        ...
                        for (cpu_num= 0; cpu_num < sys_detail.nodes[node_num].chips[chip_num].cores[core_num].num_cpus; cpu_num++) {
                            ...
                            if (num_tests_configured == 0) {
                                ...
                                for (i=0; i < num_tests_configured; i++) {
			                        ...
			                        equaliser_action (index, lcpu, pcpu);
                                }
                            }
                        }
                    }
            ...
            for (index = 0; index < (shm_addr.hdr_addr)->num_entries; index++) {
                equaliser_action (index, -1, -1);
            }
        }
        fprintf(log_fp, "-------------------------------------------------------\n");
        fflush(log_fp);
        ...
        if (test_config.sys_util_flag && test_config.sys_util_config.data.current_step == 0) {
            update_sys_util_pattern();
        }
...
#ifdef __HTX_LINUX__
     if (test_config.offline_cpu_prcnt != 0) {
		 for (node_num = 0; node_num < sys_detail.num_nodes; node_num++) {
			 for (chip_num = 0; chip_num < sys_detail.nodes[node_num].num_chips; chip_num++) {
				 for (core_num = 0; core_num < sys_detail.nodes[node_num].chips[chip_num].num_cores; core_num++) {
					 for (cpu_num = 0; cpu_num < sys_detail.nodes[node_num].chips[chip_num].cores[core_num].num_cpus; cpu_num++) {
						 pcpu = sys_detail.nodes[node_num].chips[chip_num].cores[core_num].cpus[cpu_num].pcpu;
						 sprintf (str, "echo 1 > /sys/devices/system/cpu/cpu%d/online", pcpu);
						 system(str);
					 }
				 }
			 }
